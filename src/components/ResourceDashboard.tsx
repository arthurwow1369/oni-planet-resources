...
const categoryOrder: Category[] = ['food', 'plants', 'fauna', 'oxygen', 'power', 'radiation', 'lateGame', 'industrial', 'decoration', 'liquid', 'gas', 'uncategorized']
    ...
    <section className="panel resource-panel">
      <div className="section-heading resource-heading">
        <div>
          ...
          <h2>{t.resources}</h2>
          <p>{resources.length} {t.resourceCount} · {terrains.length} {t.terrainCount}</p>
        </div>
            ...
            <section className="category-section" key={category}>
              ...
              <div className="resource-grid">
                {items.map((resource) => {
                  ...
                  const terrainNames = [...new Set(resource.terrainIds.flatMap((id) => {
                  ...
                  return (
                    <article className={`resource-card resource-card-${resourceTone(resource)}`} key={`${category}-${resource.simhash}`}>
                      ...
                      <p className={use ? 'use-copy' : 'use-copy muted'}>{use || t.unknownUse}</p>
